...
> Общий учебный поток (MVP): [learning-flow.md](./learning-flow.md).

Детали контракта `/run` и hint-таблицы ниже остаются в силе; порядок внедрения и две кнопки (локальный запуск vs проверка) — в v2.
